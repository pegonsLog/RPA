...
df_respondentes = pd.read_csv(PATH, sep='|', encoding='latin1')
...
df_estados = pd.read_csv(PATH, sep=';', encoding='latin1')
...
df_respondentes['estado_civil'] = df_respondentes['estado_civil'].fillna(
    value=moda_estado_civil
)
...
df_respondentes['idade'] = (
    (data_hoje - df_respondentes['data_nascimento']).dt.days / 365.25
).astype(int)
...
# %%
df = pd.merge(
    left=df, right=df_estados, left_on='cod_estado', right_on='cod_estado', how='inner'
)
...
# %%
cols_estados = ['cod_estado', 'sigla', 'estado', 'regiao', 'pais']
df_estados = df[cols_estados].drop_duplicates()
...
# %%
lista_educacao = list(df['educacao'].unique())
lista_educacao
...
insert_dados_bancos(lista_animais, tabela_banco)

# %%
tabela_banco = 'tb_clima'
insert_dados_bancos(lista_climas, tabela_banco)

# %%
tabela_banco = 'tb_bebida'
insert_dados_bancos(lista_bebidas, tabela_banco)

# %%
tabela_banco = 'tb_hobby'
insert_dados_bancos(lista_hobbies, tabela_banco)

# %%
tabela_banco = 'tb_preferencia_musical'
insert_dados_bancos(lista_preferencias_musicais, tabela_banco)

# %%
tabela_banco = 'tb_atividade_fisica'
insert_dados_bancos(lista_atividades_fisicas, tabela_banco)

# %%
tabela_banco = 'tb_educacao'
insert_dados_bancos(lista_educacao, tabela_banco)
...
# %%
for registro in df_estados.itertuples():
    print(registro)
...
# %%
try:
    df.to_sql('stg_pesquisa', schema='pesquisa_preferencias', index=False, con=conn)
...
# %%
conn.execute(text('use pesquisa_preferencias'))
...
# %%
query = '''
select pes.genero, est.estado, pes.idade, pes.renda_mensal
from tb_pessoa as pes
inner join tb_estado as est on est.cod_estado = pes.cod_estado
